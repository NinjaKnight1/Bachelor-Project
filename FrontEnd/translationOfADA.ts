      ...
      switch (artifact.$type) {
        case 'bpmn:Association':
          ...
          if (artifactSourceRef.sourceRef.$type == 'bpmn:ExclusiveGateway') {
            ...
            try {
              const [translatedGateText, gateGuardVariableNameSet] = translateFeelToADA(gateText, ParseType.Expression);
    ...
    switch (view.type) {
      case DecisionTableType:
        ...
        decisionLogicInfo.input.forEach((input: any) => {
          ...
          try {
            const [translatedText, inputVariableNames] = translateFeelToADA(inputText, ParseType.Expression);
        ...
        decisionLogicInfo.output.forEach((output: any) => {
          ...
          try {
            const [translatedText, inputVariableNames] = translateFeelToADA(outputName, ParseType.Expression);
            inputVariableNames.forEach(name => variableNameSet.add(name));
        ...
        for (let rowNumber = 0; rowNumber < rules.length; rowNumber++) {
          ...
          for (let inputColumn = 0; inputColumn < inputEntry.length; inputColumn++) {
            ...
            try {
              const [translatedText, inputVariableNames] = translateFeelToADA(inputText, ParseType.Unary, inputHeader.at(inputColumn)?.expression);
              inputVariableNames.forEach(name => variableNameSet.add(name));
          ...
          switch (hitPolicy) {
            ...
            case hitPolicyType.First:
              if (rowNumber == 0) {
                ...
                const notPreviousRows = '(!(' + tempPreCon + '))';
                preCondition = listWithExpression([notPreviousRows, inputRowRule], 'and');
              }
              break;
            default:
              break;
          ...
          for (let outputColumn = 0; outputColumn < outputEntry.length; outputColumn++) {
            ...
            try {
              const outputHeaderExpression = outputHeader.at(outputColumn)?.expression ?? '';
              const primedOutputHeaderExpression = outputHeaderExpression ? `${outputHeaderExpression}'` : outputHeaderExpression;
              const [translatedText, inputVariableNames] = translateFeelToADA(outputText, ParseType.Unary, primedOutputHeaderExpression);
              inputVariableNames.forEach(name => variableNameSet.add(name));
...
function translateFeelToADA(expression: string, parseType: ParseType, headerExpression: string = ''): [string, Set<string>] {
...
function walkTreeUnary(cursor: TreeCursor, expression: string, headerExpression: string, variableNameSet: Set<string>): string {
  switch (cursor.node.type.name) {
    ...
    case 'PositiveUnaryTests':
      ...
      return listWithExpression(positiveUnaryTestsList, '||');
...
export { translateFeelToADA, ParseType };